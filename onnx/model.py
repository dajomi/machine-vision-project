...
class VOCDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        image_t = TF.to_tensor(image)  # [0,1], RGB
        target = {
            "boxes": boxes_t,
            "labels": labels_t,
...
class SSDMobileNetV2Backbone(nn.Module):
    def __init__(self, img_size: int = 320, pretrained: bool = True, trainable: bool = True):
        ...
        weights = MobileNet_V2Weights = MobileNet_V2_Weights.DEFAULT if pretrained else None
...
class DetectNetONNXExportWrapper(nn.Module):
    """
    Export 2 outputs only:
      scores: [B, N, C]
      boxes : [B, N, 4] normalized xyxy in [0,1]

    detectNet ONNX path expects confidence + bbox outputs and applies
    thresholding/clustering internally, so NMS is intentionally excluded.
    """
    ...
    def forward(self, x: torch.Tensor):
        ...
        anchors = torch.stack(anchors, dim=0)  # [B,N,4]
...
def validate_one_epoch(model, loader, device, epoch: int) -> float:
    model.train()  # torchvision SSD returns losses only in train mode
    total_loss = 0.0
...
def parse_args():
    p = argparse.ArgumentParser(description="Train SSD-MobileNetV2 for detectNet deployment and save best.pth")
...
def main():
    ...
    print(f"normalization      : (x/255 - 0.5)/0.5 for detectNet alignment")
